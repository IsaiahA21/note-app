// the pages/[id] folder will handle any routing when we click edit
...
const editNote = ({ note }) => {
    ...
    useEffect(() => {
        if(isSubmitting === true){
            ...
            if(Object.keys(errors).length ===0) {
                updateNote();// run our create note method. createNote runs our http request post request
            }
            else{
    ...
    const updateNote = async() => {
        try {
            const respond = await fetch(`http://localhost:3001/api/notes/${router.query.id}`, {
                method: 'PUT',
                headers: {
...
// when the user click the edit button, we need to grab the id parameter from the url
...
editNote.getInitialProps = async ({query: {id}}) => {
    ...
    console.log('%c⧭', 'color: #00bf00','rendering the data from button edit: ' + data);
